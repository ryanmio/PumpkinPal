...
function EditPumpkin() {
  ...
  useEffect(() => {
    const fetchPumpkin = async () => {
      if (!id) return; // Ensure id is defined before attempting to fetch data
      try {
        ...
        if (pumpkinDoc.exists()) {
          ...
          toast.error("Pumpkin not found.");
          router.push('/dashboard'); // Navigate back to dashboard if pumpkin not found
        }
      } catch (error) {
